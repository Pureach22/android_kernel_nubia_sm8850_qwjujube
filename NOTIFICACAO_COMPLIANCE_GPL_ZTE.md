...
* **Xiaomi (Organização `MiCode` no GitHub):** Disponibiliza nos repositórios públicos (como `Xiaomi_Kernel_OpenSource`) todo o código-fonte dos drivers de tela física em `drivers/input/touchscreen/` (contendo os drivers de baixo nível da Goodix, Synaptics e Focaltech), bem como o gerenciamento completo de carga e reguladores de tensão em `drivers/power/supply/` para todos os seus flagships (sem stubs ou blobs pré-compilados).
* **OnePlus / OPPO / Realme (Organização `OnePlusOSS` no GitHub):** Fornecem árvores de kernel completas de produção que compilam nativamente. Isso inclui o código-fonte completo de suas tecnologias e políticas de carregamento proprietárias (VOOC/SuperVOOC/Warp Charge) sob o diretório `drivers/power/oplus_chg/` e drivers de painel sob `drivers/input/touchscreen/`.
