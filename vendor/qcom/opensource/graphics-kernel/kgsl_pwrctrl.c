...
/* Started by AICoder, pid:77f70tfd9b3aa94148e30a455115cd0c29d6b625 */
//zte add begin
void kgsl_pwrctrl_set_max_level(u32 level)
{
	...
	{
		level = pwr->min_pwrlevel;
		pr_info("%s: level=%d `s freq  is smaller than minimum level %d `s freq,change to minimum \n",__func__, level, pwr->min_pwrlevel);
	}
...
void kgsl_pwrctrl_set_min_level(u32 level)
{
	...
	{
		level = pwr->max_pwrlevel;
		pr_info("%s: level=%d `s freq is bigger than max level %d `s freq ,change to max  \n",__func__, level, pwr->max_pwrlevel);
	}
...
int kgsl_pwrctrl_get_gpu_loading(void)
{
	...
	if (stats->total_old != 0)
		ret = (stats->busy_old * 100) / stats->total_old;

	return ret;
}
//zte add end
/* Ended by AICoder, pid:77f70tfd9b3aa94148e30a455115cd0c29d6b625 */
